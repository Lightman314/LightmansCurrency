...
import io.github.lightman314.lightmanscurrency.tradedata.TicketTradeData.TicketTradeType;
...
public class TicketTraderContainer extends Container implements ITraderContainer, ITradeButtonContainer{
	...
	public TicketTraderContainer(int windowId, PlayerInventory inventory, TicketTraderTileEntity tileEntity)
	{
		this(ModContainers.TICKETTRADER, windowId, inventory, tileEntity);
	}
	
	protected TicketTraderContainer(ContainerType<?> type, int windowId, PlayerInventory inventory, TicketTraderTileEntity tileEntity)
	{
		super(type, windowId);
	...
	{
		TicketTradeData trade = tileEntity.getTrade(tradeIndex);
		if(trade == null)
			return false;
	...
	{
		return this.tileEntity.getTrade(tradeIndex).emulateItemTrade();
	}
	
	private void PostTradeEvent(TicketTradeData trade)
	{
	...
	public void ExecuteTrade(int tradeIndex)
	{
		...
		//Process a sale
		if(trade.getTradeDirection() == TicketTradeType.SALE)
		{
			
			if(InventoryUtil.GetItemTagCount(this.tileEntity.getStorage(), Reference.TICKET_MATERIAL_TAG) <= 0)
				return;
			
			//Abort if not enough room to put the sold item
			if(!InventoryUtil.CanPutItemStack(this.itemSlots, trade.getTicket()))
			{
			...
			ItemStack giveStack = trade.getTicket();
			//Give the trade item
			...
			this.tileEntity.getLogger().AddLog(player, trade.emulateItemTrade(), this.tileEntity.isCreative());
			...
			if(!this.tileEntity.isCreative())
			{
				...
				InventoryUtil.RemoveItemTagCount(this.tileEntity.getStorage(), Reference.TICKET_MATERIAL_TAG, 1);
	...
	public void UpdateTradeDisplays()
	{
		...
		{
			TicketTradeData trade = tileEntity.getTrade(i);
			if(trade != null)
