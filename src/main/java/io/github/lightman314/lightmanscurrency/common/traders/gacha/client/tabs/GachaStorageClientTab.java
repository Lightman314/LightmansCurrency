...
public class GachaStorageClientTab extends TraderStorageClientTab<GachaStorageTab> implements IScrollable, IMouseListener {
    ...
    public void initialize(ScreenArea screenArea, boolean firstOpen) {
        ...
        if(this.commonTab.isPersistent())
    ...
    public void renderBG(EasyGuiGraphics gui) {
        ...
        GachaStorageNode node = this.commonTab.getNode();
        if(node != null)
        {
            //Validate the scroll
            this.validateScroll();
            ...
            int currentCount = node.getStorage().getItemCount();
            int maxCount = node.getStorageCapacity();
    ...
    private int totalStorageSlots() {
        GachaStorageNode node = this.commonTab.getNode();
        return node == null ? 0 : node.getStorage().getContents().size();
    }
