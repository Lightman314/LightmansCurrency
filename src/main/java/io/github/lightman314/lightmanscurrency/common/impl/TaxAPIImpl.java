...
public class TaxAPIImpl extends TaxAPI {
    ...
    @Nonnull
    @Override
    public List<ITaxCollector> GetTaxCollectorsFor(@Nonnull ITaxable taxable) { return TaxDataCache.TYPE.get(taxable.isClient()).getAllEntries().stream().filter(e -> e.ShouldTax(taxable)).map(e -> (ITaxCollector)e).toList(); }

    @Nonnull
    @Override
    public List<ITaxCollector> GetPotentialTaxCollectorsFor(@Nonnull ITaxable taxable) { return TaxDataCache.TYPE.get(taxable.isClient()).getAllEntries().stream().filter(e -> e.IsInArea(taxable)).map(e -> (ITaxCollector)e).toList(); }
