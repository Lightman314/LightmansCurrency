...
public class TaxAPIImpl extends TaxAPI {
    ...
    @Override
    public List<ITaxCollector> GetTaxCollectorsFor(ITaxableContext context) { return TaxDataCache.TYPE.get(context.taxable().isClient()).getAllEntries().stream().filter(e -> e.ShouldTax(context)).map(e -> (ITaxCollector)e).toList(); }
