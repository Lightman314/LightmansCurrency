package io.github.lightman314.lightmanscurrency.common.traders.item.client.tabs;
...
public class ItemStorageClientTab extends TraderStorageClientTab<ItemStorageTab> implements IScrollable, IMouseListener {
	...
	@Override
	public Component getTooltip() { return LCText.TOOLTIP_TRADER_STORAGE.get(); }
	...
	public void renderBG(EasyGuiGraphics gui) {
        ...
        ItemStorageNode node = this.commonTab.getNode();
	...
	private int totalStorageSlots() {
        ItemStorageNode node = this.commonTab.getNode();
		if(node != null)
			return node.getStorage().getContents().size();
		return 0;
	}
	...
	{
        ItemStorageNode node = this.commonTab.getNode();
		return node != null && !this.commonTab.isPersistent();
	}
	...
	public Pair<ItemStack, ScreenArea> getHoveredItem(ScreenPosition mousePos) {
		...
		if(node != null) {
			...
			ItemStack stack = node.getStorage().getStackInSlot(slot);
