...
public class BlockValidator extends MenuValidator {
    ...
    private static final class Type extends MenuValidatorType
    {
        ...
        @Nonnull
        @Override
        public MenuValidator decode(@Nonnull FriendlyByteBuf buffer) { return of(buffer.readBlockPos(), ForgeRegistries.BLOCKS.getValue(VersionUtil.parseResource(buffer.readUtf()))); }
        @Nonnull
        @Override
        public MenuValidator load(@Nonnull CompoundTag tag) { return of(TagUtil.loadBlockPos(tag.getCompound("Position")), ForgeRegistries.BLOCKS.getValue(VersionUtil.parseResource(tag.getString("Block")))); }
    }
