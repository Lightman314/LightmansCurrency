...
public abstract class ItemsForXTradeTemplate implements VillagerTrades.ItemListing {
    ...
    protected final void serializeData(@Nonnull JsonObject json)
    {
        json.add("Price", FileUtil.convertItemStack(this.price));
        if(!this.price2.isEmpty())
            json.add("Price2", FileUtil.convertItemStack(this.price2));
        json.addProperty("MaxTrades", this.maxTrades);
        json.addProperty("XP", this.xp);
        json.addProperty("PriceMult", this.priceMult);
    }

    protected static DeserializedData deserializeData(@Nonnull JsonObject json) throws JsonSyntaxException, ResourceLocationException
    {
