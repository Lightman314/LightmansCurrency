...
public class DateTrigger extends SimpleCriterionTrigger<DateTrigger.Instance> {
    ...
    public static final ResourceLocation ID = LightmansCurrency.id("date_range");
    ...
    public static Criterion<DateTrigger.Instance> ofRange(DatePredicate startDate, DatePredicate endDate) { return INSTANCE.createCriterion(new Instance(Optional.empty(), startDate, endDate)); }

    
    @Override
    public Codec<Instance> codec() { return CODEC; }
    public void trigger(ServerPlayer player) { this.trigger(player, Instance::test); }
