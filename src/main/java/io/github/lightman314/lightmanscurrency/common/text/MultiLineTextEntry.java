...
public final class MultiLineTextEntry {
    ...
    public static MultiLineTextEntry notification(ResourceLocation type) { return notification(() -> type); }
    public static MultiLineTextEntry notification(Supplier<ResourceLocation> type) { return delayed(type,key -> "notification." + key.getNamespace() + "." + key.getPath()); }

    private static MultiLineTextEntry delayed(Supplier<ResourceLocation> key, Function<ResourceLocation,String> factory) { return new MultiLineTextEntry(() -> factory.apply(key.get())); }
