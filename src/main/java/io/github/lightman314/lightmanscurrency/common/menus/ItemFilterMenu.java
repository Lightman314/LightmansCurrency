...
public class ItemFilterMenu extends LazyMessageMenu {
    ...
    public FilterData getData() { return this.getTargetedStack().getOrDefault(ModDataComponents.FILTER_DATA,FilterData.EMPTY); }
    ...
    protected final void editData(UnaryOperator<FilterData> action)
    {
        ...
        {
            FilterData filter = target.getOrDefault(ModDataComponents.FILTER_DATA,FilterData.EMPTY);
            FilterData newFilter = action.apply(filter);
            if(newFilter.equals(filter))
                return;
            target.set(ModDataComponents.FILTER_DATA,newFilter);
        }
    }
    ...
    {
        player.openMenu(new Provider(itemIndex),buf -> buf.writeInt(itemIndex));
    }
