...
public class CommandBank {
    ...
    static int giveTo(CommandSourceStack source, List<BankAccount.AccountReference> accounts, CoinValue amount)
    {
        ...
        {
            if(count == 1)
                source.sendSuccess(Suppliers.ofInstance(EasyText.translatable("command.lightmanscurrency.lcbank.give.success.single", amount.getComponent("NULL"), bankName)), true);
            else
                source.sendSuccess(Suppliers.ofInstance(EasyText.translatable("command.lightmanscurrency.lcbank.give.success", amount.getComponent("NULL"), count)), true);
        }
        return count;
    }
    ...
    static int takeFrom(CommandSourceStack source, List<BankAccount.AccountReference> accounts, CoinValue amount)
    {
        ...
        {
            if(count == 1)
                source.sendSuccess(Suppliers.ofInstance(EasyText.translatable("command.lightmanscurrency.lcbank.take.success.single", largestAmount.getComponent("NULL"), bankName)), true);
            else
                source.sendSuccess(Suppliers.ofInstance(EasyText.translatable("command.lightmanscurrency.lcbank.take.success", largestAmount.getComponent("NULL"), count)), true);
        }
        return count;
    }
