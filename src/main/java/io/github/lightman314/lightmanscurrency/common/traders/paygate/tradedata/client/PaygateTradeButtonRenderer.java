...
public class PaygateTradeButtonRenderer extends TradeRenderManager<PaygateTradeData> {
    ...
    public List<TradeButton.DisplayEntry> getInputDisplays(TradeContext context) {
        if(this.trade.isTicketTrade())
            return Lists.newArrayList(TradeButton.DisplayEntry.of(TicketItem.CreateTicket(this.trade.getTicketID(), this.trade.getTicketColor()), 1, Lists.newArrayList(EasyText.translatable("tooltip.lightmanscurrency.ticket.id", this.trade.getTicketID()))));
        else
