...
public class CoinBlock extends FallingBlock {
	...
	@Nullable
	protected LootTable getLootTable(Level level, BlockState state)
	{
        AtomicReference<LootTable> table = new AtomicReference<>(null);
        Optional<Holder.Reference<LootTable>> op = level.registryAccess().lookupOrThrow(Registries.LOOT_TABLE).get(ResourceKey.create(Registries.LOOT_TABLE,this.getLootTableLocation(state)));
        if(op.isPresent())
            return op.get().value();
        return null;
	}
	...
	protected ResourceLocation getLootTableLocation(BlockState state)
	{
