...
public final class TextEntryBiBundle<S,T> {
    ...
    public TextEntryBiBundle(Map<S,Map<T,TextEntry>> map)
    {
    ...
    public static <S,T> TextEntryBiBundle<S,T> of(RegistryObjectBiBundle<? extends ItemLike,S,T> bundle)
    {
    ...
    public TextEntry get(S key1, T key2)
    {
        Map<T,TextEntry> map = this.entryMap.getOrDefault(key1, new HashMap<>());
        return map.get(key2);
    }

    public void forEach(TriConsumer<S,T,TextEntry> consumer)
    {
