package io.github.lightman314.lightmanscurrency.common.traders.paygate.client.trade;
...
public class PaygateTradeButtonRenderer extends TradeRenderManager<PaygateTradeData> {
    ...
    public List<DisplayEntry> getInputDisplays(TradeContext context) {
        if(this.trade.isTicketTrade())
            return Lists.newArrayList(ItemEntry.of(TicketItem.CreateTicket(this.trade.getTicketItem(), this.trade.getTicketID(), this.trade.getTicketColor()), LCText.TOOLTIP_TICKET_ID.getAsList(this.trade.getTicketID())));
        else
    ...
    @Override
    public List<DisplayEntry> getOutputDisplays(TradeContext context) { return Lists.newArrayList(this.trade.getDescription().isBlank() ? TextDisplayEntry.of(PaygateTradeData.formatDurationDisplay(this.trade.getDuration()), TextRenderUtil.TextFormatting.create(), Lists.newArrayList(PaygateTradeData.formatDuration(this.trade.getDuration()),LCText.GUI_TRADER_PAYGATE_LEVEL.get(this.trade.getRedstoneLevel()))) : ScrollingTextEntry.of(EasyText.literal(this.trade.getDescription()),0xFFFFFF,TooltipHelper.splitTooltips(this.trade.getDescriptionTooltip()))); }
    ...
    protected void getAdditionalAlertData(TradeContext context, List<AlertData> alerts) {
        if(context.getTrader() instanceof PaygateTraderData paygate)
        {
            ...
            {
                alerts.add(AlertData.of(LCText.TOOLTIP_TRADER_PAYGATE_ALREADY_ACTIVE.get(), handling.allowsConflicts ? AlertType.NEUTRAL : AlertType.WARN));
