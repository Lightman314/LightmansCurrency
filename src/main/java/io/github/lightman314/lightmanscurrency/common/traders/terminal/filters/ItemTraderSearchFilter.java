...
public class ItemTraderSearchFilter implements ITraderSearchFilter, ITradeSearchFilter {
		...
		if(data instanceof ItemTradeData trade)
		{
			if (trade.isValid()) {
				...
				if (!sellItem2.isEmpty() && trade.getCustomName(1).toLowerCase().contains(searchText))
					return true;
				//Search enchantments
				if(searchEnchantments(sellItem,searchText,lookup))
					return true;
				if(searchEnchantments(sellItem2,searchText,lookup))
					return true;
				...
				if (trade.isBarter()) {
					...
					if (!barterItem2.isEmpty() && barterItem2.getHoverName().getString().toLowerCase().contains(searchText))
						return true;
					//Search enchantments
					if(searchEnchantments(barterItem,searchText,lookup))
						return true;
					if(searchEnchantments(barterItem2,searchText,lookup))
						return true;
				}
			}
		}
	...
	private static boolean searchEnchantments(@Nonnull ItemStack stack, @Nonnull String searchText, @Nonnull HolderLookup.Provider lookup)
	{
		ItemEnchantments enchantments = stack.getAllEnchantments(lookup.lookupOrThrow(Registries.ENCHANTMENT));
		for(var enchantment : enchantments.entrySet())
		{
			if(Enchantment.getFullname(enchantment.getKey(),enchantment.getIntValue()).getString().toLowerCase().contains(searchText))
				return true;
		}
		return false;
	}
