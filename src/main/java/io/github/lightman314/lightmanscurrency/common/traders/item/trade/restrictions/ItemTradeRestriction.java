...
public class ItemTradeRestriction {
	...
	public static final ResourceLocation NO_RESTRICTION_KEY = LightmansCurrency.id("none");
	...
	private static void register(String type, ItemTradeRestriction restriction) { register(LightmansCurrency.id(type), restriction); }
    ...
    @Nullable
    protected final List<ItemStack> getRandomSellItems(TraderData trader, RequirementWithContext... requirements) { return getRandomSellItems(trader, Lists.newArrayList(requirements)); }
    @Nullable
    protected final List<ItemStack> getRandomSellItems(TraderData trader, List<RequirementWithContext> requirements)
    {
        ...
        if(randomItems == null && trader.hasInfiniteStock()) //If creative, return nbt enforced version if no random items are present in the items.
        {
            ...
            randomItems = ItemHandlerUtil.combineStacks(randomItems);
        }
        return randomItems;
    }
	...
	protected final void removeFromStorage(ItemStack sellItem, TraderItemStorage traderStorage)
	{
		if(sellItem.isEmpty())
			return;
		traderStorage.removeItemLimited(sellItem);
	}
