...
public class UpgradeContainer extends SimpleContainer {
    ...
    public void save(String key, CompoundTag tag, HolderLookup.Provider lookup) { InventoryUtil.saveAllItems(key,tag,this,lookup); }
    public void load(String key, CompoundTag tag, HolderLookup.Provider lookup)
    {
        if(tag.contains(key))
            this.copyContents(InventoryUtil.loadAllItems(key,tag,this.getContainerSize(),lookup));
    }
