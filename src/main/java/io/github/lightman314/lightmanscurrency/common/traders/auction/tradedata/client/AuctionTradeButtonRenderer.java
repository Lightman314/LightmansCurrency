...
public class AuctionTradeButtonRenderer extends TradeRenderManager<AuctionTradeData> {
    ...
    private List<ITextComponent> getBidInfo() {
        List<ITextComponent> bidInfo = new ArrayList<>();
    ...
    @Override
    public List<ITextComponent> getAdditionalTooltips(TradeContext context, int mouseX, int mouseY) {
        TimeUtil.TimeData time = new TimeUtil.TimeData(this.trade.getRemainingTime(TimeUtil.getCurrentTime()));
        return Lists.newArrayList(EasyText.translatable("gui.lightmanscurrency.auction.time_remaining", EasyText.literal(time.getString()).withStyle(s -> s.withColor(Color.fromRgb(this.getTextColor(time))))));
    }
