...
public class BlockEntityValidator extends MenuValidator {
    ...
    @Override
    public boolean stillValid(@Nonnull Player player) { this.validateBE(player); return this.be != null && stillValidBlockEntity(this.be, player); }
    ...
    private static boolean stillValidBlockEntity(@Nonnull BlockEntity blockEntity, @Nonnull Player player) {
        ...
        if (level == null) {
            ...
            return player.distanceToSqr((double)blockpos.getX() + 0.5D, (double)blockpos.getY() + 0.5D, (double)blockpos.getZ() + 0.5D) <= 64;
        }
    }
