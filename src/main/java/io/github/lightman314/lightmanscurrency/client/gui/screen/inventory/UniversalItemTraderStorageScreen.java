...
public class UniversalItemTraderStorageScreen extends ContainerScreen<UniversalItemTraderStorageContainer>{
	...
	public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks)
	{
		...
		else if(this.container.player.inventory.getItemStack().isEmpty())
		{
			...
			for(int i = 0; i < this.container.getData().getTradeCount(); i++)
			{
				...
				int result = ItemTradeButton.tryRenderTooltip(matrixStack, this, i, this.container.getData(), this.guiLeft + ItemTraderStorageUtil.getFakeTradeButtonPosX(tradeCount, i), this.guiTop + ItemTraderStorageUtil.getFakeTradeButtonPosY(tradeCount, i), inverted, mouseX, mouseY);
	...
	public void tick()
	{
		...
		this.buttonCollectMoney.visible = (!this.container.getData().getCoreSettings().isCreative() || this.container.getData().getStoredMoney().getRawValue() > 0) && this.container.hasPermission(Permissions.COLLECT_COINS) && !this.container.getData().getCoreSettings().hasBankAccount();;
