...
public class MainTab extends SettingsTab{
	...
	public void initTab() {
		...
		if(this.getScreen().getTrader() instanceof UniversalTraderData)
		{
			...
			int idWidth = this.getFont().width(Component.translatable("gui.lightmanscurrency.settings.persistent.id"));
			this.persistentTraderIDInput = screen.addRenderableTabWidget(new EditBox(this.getFont(), screen.guiLeft() + 37 + idWidth, screen.guiTop() + screen.ySize - 30, 108 - idWidth, 18, Component.empty()));
			
			int ownerWidth = this.getFont().width(Component.translatable("gui.lightmanscurrency.settings.persistent.owner"));
			this.persistentTraderOwnerInput = screen.addRenderableTabWidget(new EditBox(this.getFont(), screen.guiLeft() + 12 + ownerWidth, screen.guiTop() + screen.ySize - 55, 178 - ownerWidth, 18, Component.empty()));
	...
	public void preRender(PoseStack pose, int mouseX, int mouseY, float partialTicks) {
		...
		if(TradingOffice.isAdminPlayer(this.getScreen().getPlayer()))
		{
			...
			{
				//Draw ID input label
				this.getFont().draw(pose, Component.translatable("gui.lightmanscurrency.settings.persistent.id"), screen.guiLeft() + 35, screen.guiTop() + screen.ySize - 25, 0xFFFFFF);
				//Draw Owner input label
				this.getFont().draw(pose, Component.translatable("gui.lightmanscurrency.settings.persistent.owner"), screen.guiLeft() + 10, screen.guiTop() + screen.ySize - 50, 0xFFFFFF);
	...
	private void SavePersistentTraderData(Button button)
	{
		try {
			...
			if(t instanceof UniversalTraderData) {
				...
				/* Old method of exporting to Json and copying to the clipboard.
