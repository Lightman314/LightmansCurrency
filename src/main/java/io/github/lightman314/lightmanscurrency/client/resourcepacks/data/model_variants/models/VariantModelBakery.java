...
public class VariantModelBakery {
    ...
    public void bakeModels(BiFunction<String,Material,TextureAtlasSprite> textureGetter)
    {
        this.topLevelModels.forEach((id,unbakedModel) -> {
            ...
            try {
                bakedmodel = new VariantModelBaker(id,textureGetter).bakeFromKey(id,unbakedModel,BlockModelRotation.X0_Y0);
            } catch (Exception e) {
    ...
    private class VariantModelBaker implements ModelBaker
    {
        ...
        public BakedModel bake(ResourceLocation location, ModelState state, Function<Material, TextureAtlasSprite> sprites) {
            ...
            if (unbakedmodel instanceof BlockModel blockModel) {
                if (blockModel.getRootModel() == ModelBakery.GENERATION_MARKER) {
                    return ITEM_MODEL_GENERATOR.generateBlockModel(sprites, blockModel).bake(this,blockModel,sprites,state,location, false);
                }
            }
            return unbakedmodel.bake(this,sprites,state,location);
        }
        ...
        public BakedModel bakeFromKey(ModelCacheKey key, UnbakedModel model, ModelState state)
        {
            if (model instanceof BlockModel blockModel) {
                if (blockModel.getRootModel() == ModelBakery.GENERATION_MARKER) {
                    return ITEM_MODEL_GENERATOR.generateBlockModel(this.textureGetter, blockModel).bake(this,blockModel,this.textureGetter,state,key.model,false);
                }
            }
            return model.bake(this,this.textureGetter,state,key.model);
        }
    ...
    public record ModelProperties(int requiredModels, int modelsFromBlockState, boolean isRotatable, ResourceLocation target, Function<Integer,ResourceLocation> defaultModelSource, Function<BlockState,Integer> stateIndexLookup)
    {
        ...
        private static ModelProperties collectProperties(ModelVariant variant, ResourceLocation target)
        {
            if(variant.isItemVariant())
            {
                IVariantItem item = VariantProvider.getVariantItem(ForgeRegistries.ITEMS.getValue(target));
                if(item != null)
                    return new ModelProperties(item.requiredModels(),0,false,item.getItemID(),item::getDefaultModel,s -> -1);
            }
            else
            {
                IVariantBlock block = VariantProvider.getVariantBlock(ForgeRegistries.BLOCKS.getValue(target));
                if(block != null)
