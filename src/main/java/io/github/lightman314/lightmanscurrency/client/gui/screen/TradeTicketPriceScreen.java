...
import io.github.lightman314.lightmanscurrency.network.message.ticket_trader.MessageSetItemPrice3;
...
public class TradeTicketPriceScreen extends Screen implements ICoinValueInput{
	...
	TicketTradeType localDirection;
	
	CoinValueInput priceInput;
	
	TextFieldWidget nameField;
	
	public TradeTicketPriceScreen(TicketTraderTileEntity tileEntity, int tradeIndex, PlayerEntity player)
	{
	...
	{
		LightmansCurrencyPacketHandler.instance.sendToServer(new MessageSetItemPrice3(tileEntity.getPos(), this.tradeIndex, this.priceInput.getCoinValue(), false, this.nameField.getText(), this.localDirection.name()));
	}
	...
	private static class CloseRuleHandler implements ITradeRuleScreenHandler
	{
		...
		final TicketTraderTileEntity tileEntity;
		final int tradeIndex;
		final PlayerEntity player;
		
		public CloseRuleHandler(TicketTraderTileEntity tileEntity, int tradeIndex, PlayerEntity player)
		{
			this.tileEntity = tileEntity;
			this.tradeIndex = tradeIndex;
			this.player = player;
		...
		public void reopenLastScreen() {
			Minecraft.getInstance().displayGuiScreen(new TradeTicketPriceScreen(this.tileEntity, this.tradeIndex, this.player));
		}
		
		public void updateServer(List<TradeRule> newRules)
		{
			LightmansCurrencyPacketHandler.instance.sendToServer(new MessageSetTraderRules3(this.tileEntity.getPos(), newRules, this.tradeIndex));
		}
