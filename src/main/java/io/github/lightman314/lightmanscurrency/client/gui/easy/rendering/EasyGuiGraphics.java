...
public final class EasyGuiGraphics extends GuiComponent {
    ...
    public static EasyGuiGraphics create(@Nonnull ScreenEvent.DrawScreenEvent event) { return new EasyGuiGraphics(event.getPoseStack(), event.getScreen().getMinecraft().font, event.getMouseX(), event.getMouseY(), event.getPartialTicks()); }
    public static EasyGuiGraphics create(@Nonnull ContainerScreenEvent.DrawBackground event) { return new EasyGuiGraphics(event.getPoseStack(), event.getContainerScreen().getMinecraft().font, event.getMouseX(), event.getMouseY(), 0f); }
