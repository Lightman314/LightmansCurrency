...
public final class EasyGuiGraphics extends GuiComponent {
    ...
    public void blitSpriteFadeHoriz(@Nonnull Sprite sprite, int x, int y, float percent, boolean hovered) {
        int blitWidth = MathUtil.clamp((int)(sprite.width * percent), 0, sprite.width);
    ...
    public void renderButtonBG(int x, int y, int width, int height, float alpha, int textureY)
    {
        ...
        blitNineSliced(this.pose, this.offset.x + x, this.offset.y + y, width, height, 20, 4, 200, 20, 0, textureY);
        this.resetColor();
    }
    ...
    public void drawWordWrap(String text, int x, int y, int columnWidth, int color) { this.font.drawWordWrap(this.pose, EasyText.literal(text), this.offset.x + x, this.offset.y + y, columnWidth, color); }
    public void drawWordWrap(Component text, int x, int y, int columnWidth, int color) { this.font.drawWordWrap(this.pose, text, this.offset.x + x, this.offset.y + y, columnWidth, color); }
    ...
    public void renderItem(@Nonnull ItemStack item, int x, int y, @Nullable String countTextOverride) {
        ...
        RenderSystem.enableDepthTest();
        itemRenderer.renderAndDecorateItem(this.pose, item, this.offset.x + x, this.offset.y + y, 0);
        itemRenderer.renderGuiItemDecorations(this.pose, this.font, item, this.offset.x + x, this.offset.y + y, countTextOverride);
    }
