...
public class PlayerTradeLimitTab extends TradeRuleSubTab<PlayerTradeLimit> {
    ...
    public void onOpen() {
        ...
        this.limitInput = this.addWidget(new EditBox(this.font, this.screen.getGuiLeft() + 10, this.screen.getGuiTop() + 19, 30, 20, Component.empty()));
        this.limitInput.setMaxLength(3);
        PlayerTradeLimit rule = this.getRule();
        if(rule != null)
            this.limitInput.setValue(Integer.toString(rule.getLimit()));

        this.buttonSetLimit = this.addWidget(Button.builder(Component.translatable("gui.button.lightmanscurrency.playerlimit.setlimit"), this::PressSetLimitButton).pos(this.screen.getGuiLeft() + 41, this.screen.getGuiTop() + 19).size(40, 20).build());
        this.buttonClearMemory = this.addWidget(Button.builder(Component.translatable("gui.button.lightmanscurrency.playerlimit.clearmemory"), this::PressClearMemoryButton).pos(this.screen.getGuiLeft() + 10, this.screen.getGuiTop() + 50).size(this.screen.getXSize() - 20, 20).build());
    ...
    public void renderTooltips(@Nonnull PoseStack pose, int mouseX, int mouseY) {
        ...
        if(this.buttonClearMemory.isMouseOver(mouseX, mouseY))
            this.screen.renderTooltip(pose, Component.translatable("gui.button.lightmanscurrency.playerlimit.clearmemory.tooltip"), mouseX, mouseY);
