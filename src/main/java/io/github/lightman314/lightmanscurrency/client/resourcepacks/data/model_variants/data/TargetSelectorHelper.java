...
public final class TargetSelectorHelper {
    ...
    {
        lookup(targetSelectors,targets,models,ITEM_RESULTS,ForgeRegistries.ITEMS,i -> {
    ...
    {
        lookup(targetSelectors,targets,models,BLOCK_RESULTS,ForgeRegistries.BLOCKS,b -> {
            IVariantBlock block = VariantProvider.getVariantBlock(b);
            return block == null ? -1 : block.requiredModels();
        });
    }

    private static <T> void lookup(List<String> targetSelectors, List<ResourceLocation> targets, int models, Map<SelectorKey,List<ResourceLocation>> resultCache, IForgeRegistry<T> registry, Function<T,Integer> modelRequirementLookup)
    {
