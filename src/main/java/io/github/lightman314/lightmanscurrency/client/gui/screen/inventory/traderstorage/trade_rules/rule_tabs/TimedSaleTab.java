...
public class TimedSaleTab extends TradeRuleSubTab<TimedSale> {
    ...
    public void onOpen() {
        ...
        this.discountInput = this.addWidget(new EditBox(this.font, this.screen.getGuiLeft() + 25, this.screen.getGuiTop() + 9, 20, 20, EasyText.empty()));
        ...
        this.buttonSetDiscount = this.addWidget(EasyButton.builder(EasyText.translatable("gui.button.lightmanscurrency.discount.set"), this::PressSetDiscountButton).pos(this.screen.getGuiLeft() + 125, this.screen.getGuiTop() + 10).size(50, 20).build());

        this.buttonStartSale = this.addWidget(EasyButton.builder(this.getButtonText(), this::PressStartButton).pos(this.screen.getGuiLeft() + 25, this.screen.getGuiTop() + 45).size(156, 20).build());
    ...
    public void renderBG(@Nonnull PoseStack pose, int mouseX, int mouseY, float partialTicks) {
        ...
        this.font.draw(pose, EasyText.translatable("gui.lightmanscurrency.discount.tooltip"), this.discountInput.x + this.discountInput.getWidth() + 4, this.discountInput.y + 3, 0xFFFFFF);
