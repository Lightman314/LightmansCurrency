...
public class ModelVariantDataManager implements PreparableReloadListener {
    ...
    private ModelVariantDataManager() {
        //Map<ResourceLocation, ResourceLocation> VANILLA_ATLASES = ClientHooks.gatherMaterialAtlases(Map.of(InventoryMenu.BLOCK_ATLAS,ResourceLocation.withDefaultNamespace("blocks")));
    ...
    public CompletableFuture<Void> reload(PreparationBarrier preparationBarrier, ResourceManager resourceManager, ProfilerFiller preparationsProfiler, ProfilerFiller reloadProfiler, Executor backgroundExecutor, Executor gameExecutor) {
        ...
        CompletableFuture<Map<ResourceLocation, List<BlockStateModelLoader.LoadedJson>>> blockStateFuture = loadBlockStates(resourceManager,backgroundExecutor);
    ...
    private static CompletableFuture<Map<ResourceLocation, List<BlockStateModelLoader.LoadedJson>>> loadBlockStates(ResourceManager resourceManager, Executor executor) {
        return CompletableFuture.supplyAsync(() -> BlockStateModelLoader.BLOCKSTATE_LISTER.listMatchingResourceStacks(resourceManager), executor).thenCompose((resourceMap) -> {
            List<CompletableFuture<Pair<ResourceLocation, List<BlockStateModelLoader.LoadedJson>>>> list = new ArrayList<>(resourceMap.size());
