...
public class ItemTraderBlockEntityRenderer implements BlockEntityRenderer<ItemTraderBlockEntity>{
				...
				if(!renderItems.isEmpty())
				{
					...
					for(int pos = 0; pos < renderLimit && pos < positions.size() && pos < trader.getTradeStock(tradeSlot); pos++)
					{
						...
						if(renderItems.size() > 1)
						{
							...
							renderItem(blockEntity,itemRenderer,renderItems.get(0),lightLevel,pose,buffer,id);
	...
	public static Quaternionf getRotation(float partialTicks) { return new Quaternionf().fromAxisAngleDeg(new Vector3f(0f, 1f,0f), (getRotationTime() + partialTicks) * 2.0F); }
