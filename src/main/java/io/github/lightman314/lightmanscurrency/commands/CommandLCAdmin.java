...
public class CommandLCAdmin {
	...
	static int listUniversalData(CommandContext<CommandSource> commandContext) throws CommandSyntaxException{
		...
		{
			source.sendFeedback(new TranslationTextComponent("command.lightmanscurrency.lcadmin.universaldata.list.title"), false);
			...
			for(int i = 0; i < allTraders.size(); i++)
			{
				...
				//Type
				source.sendFeedback(new TranslationTextComponent("command.lightmanscurrency.lcadmin.universaldata.list.type", thisTrader.getDeserializerType()), false);
				//Owner / Owner ID
				source.sendFeedback(new TranslationTextComponent("command.lightmanscurrency.lcadmin.universaldata.list.owner", thisTrader.getOwnerName(), thisTrader.getOwnerID().toString()), false);
				//Dimension
				String dimension = thisTrader.getWorld().getLocation().toString();
				source.sendFeedback(new TranslationTextComponent("command.lightmanscurrency.lcadmin.universaldata.list.dimension", dimension), false);
				//Position
				...
				if(thisTrader.hasCustomName())
					source.sendFeedback(new TranslationTextComponent("command.lightmanscurrency.lcadmin.universaldata.list.name", thisTrader.getName()), false);
			}
		}
		else
		{
			source.sendFeedback(new TranslationTextComponent("command.lightmanscurrency.lcadmin.universaldata.list.none"), false);
		}
