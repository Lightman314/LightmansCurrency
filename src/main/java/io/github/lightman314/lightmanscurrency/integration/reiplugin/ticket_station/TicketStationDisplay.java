...
public class TicketStationDisplay implements Display {
    ...
    public TicketStationDisplay(RecipeHolder<TicketStationRecipe> holder) {
        this.inputs = Lists.newArrayList(getModifierInput(holder.value()),
                EntryIngredients.ofIngredient(holder.value().getIngredient()));
        this.outputs = Collections.singletonList(EntryIngredients.of(holder.value().exampleResult()));
        this.location = Optional.of(holder.id());
        this.codeInputs = holder.value().requiredCodeInput();
        this.durabilityData = holder.value().getDurabilityData();
    }
