...
public class MoneyUtil {
    	...
    	if(duplicateDependentOwner != null)
    	{
    		...
    		if(duplicateDependentOwner.worthOtherCoinCount.get() > newCoinData.worthOtherCoinCount.get())
    		{
    			...
    			if(ratio % 1f == 0f)
    			{
    				...
    				final CoinData ddo = duplicateDependentOwner;
    		...
    		else
    		{
    			...
    			if(ratio % 1f == 0f)
    			{
    				//Ratio is exact, multiply the larger one by this number and all the values will be handled properly.
    				final CoinData ddo = duplicateDependentOwner;
    				newCoinData.worthOtherCoinCount = () -> newCoinData.worthOtherCoinCount.get() / ddo.worthOtherCoinCount.get();
    				if(!ignoreDuplicateWarning)
    				{
    ...
    public static class CoinData
    {
    	...
    	{
    		if(newOtherCoin == this.worthOtherCoin && newOtherCoinAmount.get() == this.worthOtherCoinCount.get())
    		{
    	...
    	public static class Builder
    	{
    		...
    		/**
    		 * Defines what lesser coin can be converted into this one, and how many of those coins are worth 1 of this coin.
    		 * Supplier input allows inputs defined by a config value
    		 */
    ...
    public static class CoinValue
    {
    	...
    	public static CoinValue Parse(JsonElement json) throws Exception
    	{
    		...
    		throw new Exception("Coin Value entry (" + json.getClass() + ") is not a valid Json Element.");
    	}
