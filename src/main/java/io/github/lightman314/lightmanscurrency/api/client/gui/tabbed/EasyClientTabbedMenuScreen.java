...
public abstract class EasyClientTabbedMenuScreen<M extends AbstractContainerMenu,S extends EasyClientTabbedMenuScreen<M,S,T>,T extends EasyClientTab<M,S,T>> extends EasyClientTab.ClientMenuScreen<M,S,T> {
    ...
    protected final void initialize(ScreenArea screenArea) {
        ...
        IWidgetPositioner tabPositioner = this.getTabButtonPositioner();
        tabPositioner.clear();
        this.addChild(tabPositioner);
        //Sort the tab buttons so that they'll be ordered by the positioner correctly
        List<Map.Entry<Integer,T>> sortedTabs = new ArrayList<>(this.menuTabs.entrySet());
        ...
        for(var entry : sortedTabs) {
            TabButton button = TabButton.builder()
                    ...
                    .addon(EasyAddonHelper.visibleCheck(entry.getValue()::tabVisible))
                    .build();
