...
public abstract class EasyButton extends EasyWidget {
    ...
    protected EasyButton(EasyButtonBuilder<?> builder) { super(builder); this.press = builder.action; this.altPress = builder.altAction; }
    ...
    @Override
    public void playDownSound(SoundManager manager) { playClick(manager); }

    public static void playClick(SoundManager manager) { manager.play(SimpleSoundInstance.forUI(SoundEvents.UI_BUTTON_CLICK, 1.0F)); }
    ...
    public static abstract class EasyButtonBuilder<T extends EasyButtonBuilder<T>> extends EasyBuilder<T>
    {
        ...
        public final T pressAction(Consumer<EasyButton> action) { this.action = action; return this.getSelf(); }
        public final T pressAction(Runnable action) { return this.pressAction(b -> action.run()); }

        public final T altPressAction(Consumer<EasyButton> action) { this.altAction = action; return this.getSelf(); }
        public final T altPressAction(Runnable action) { return this.altPressAction(b -> action.run()); }

        public final T copyFrom(EasyButtonBuilder<?> other)
        {
