...
public record SpriteSource(ResourceLocation texture, int u, int v, int width, int height, int textureWidth, int textureHeight) {
    ...
    public static SpriteSource copy(SafeSpriteData data) { return new SpriteSource(data.texture(),data.u(),data.v(),data.width(),data.height(),data.textureWidth(),data.textureHeight()); }
