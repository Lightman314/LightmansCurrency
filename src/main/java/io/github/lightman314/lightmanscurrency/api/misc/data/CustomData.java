...
public abstract class CustomData implements IClientTracker, LazyPacketData.IBuilderProvider {
    ...
    /**
     * Called in {@link net.minecraft.world.level.saveddata.SavedData#save(CompoundTag)} to save the data to disk
     * @param tag The NBT tag to write the data too
     */
    public abstract void save(CompoundTag tag);

    public final void loadData(CompoundTag tag)
    {
     ...
     * @param tag The NBT tag the data was written to in {@link #save(CompoundTag)}
     */
     ...
     * @param message The message sent from the server to sync the data with your client
     */
    protected abstract void parseSyncPacket(LazyPacketData message);

    /**
     * Called on the logical server when the data is first created/loaded from file<br>
     * Called <b>AFTER</b> {@link #load(CompoundTag)} is called, so all data should be up-to-date
     */
     ...
     * Packets will be handled client-side by {@link #parseSyncPacket(LazyPacketData)}
     */
