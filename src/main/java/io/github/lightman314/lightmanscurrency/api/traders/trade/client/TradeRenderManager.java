...
public abstract class TradeRenderManager<T extends TradeData> {
    ...
    public static void intialize()
    {
        if(renderManagers != null)
            return;
        renderManagers = ModLoader.get().postEventWithReturn(new RegisterTradeRenderManagersEvent()).getResults();
    }
    ...
    protected final DisplayEntry lazyPriceDisplay(TradeContext context,List<Component> extraTooltips)
    {
        if(context.isStorageMode && this.hasPermission(context, Permissions.EDIT_TRADES))
        {
            ...
            extraTooltips.add(0,LCText.TOOLTIP_TRADE_EDIT_PRICE.getWithStyle(ChatFormatting.YELLOW));
        }
