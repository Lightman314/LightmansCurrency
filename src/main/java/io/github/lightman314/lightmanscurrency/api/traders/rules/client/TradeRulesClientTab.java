...
import io.github.lightman314.lightmanscurrency.api.traders.rules.client.builtin.RuleToggleTab;
...
public abstract class TradeRulesClientTab<T extends TradeRulesTab> extends TraderStorageClientTab<T> {
    ...
    public static final ClientRegistry<TradeRuleType<?>,Function<TradeRulesClientTab<?>,TradeRulesClientSubTab>> REGISTRY = new ClientRegistry<>(LCRegistries.TRADE_RULE,"Trade Rule Client Tab Builder");
    ...
    public void refreshTabs(boolean fullReset)
    {
        ...
        if(host != null)
        {
            ...
            {
                try{
                    var builder = REGISTRY.getOrThrow(rule.getType());
                    if(builder != null)
                        this.tabs.add(builder.apply(this));
                } catch(Throwable t) {
                    LightmansCurrency.LogError("Trade Rule of type '" + rule.getType() + "' encountered an error creating its tab. Trade Rule will not be editable!", t);
                }
            }
        }
    ...
    @Override
    public void renderBG(EasyGuiGraphics gui) { this.getCurrentTab().renderBG(gui); }
