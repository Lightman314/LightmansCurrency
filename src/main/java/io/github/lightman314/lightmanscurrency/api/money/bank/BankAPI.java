...
public abstract class BankAPI {
    ...
    public abstract void RegisterBankAccountSource(BankAccountSource source);

    public abstract List<IBankAccount> GetAllBankAccounts(boolean isClient);
    public final List<IBankAccount> GetAllBankAccounts(IClientTracker context) { return this.GetAllBankAccounts(context.isClient()); }
    public abstract List<BankReference> GetAllBankReferences(boolean isClient);
    public final List<BankReference> GetAllBankReferences(IClientTracker context) { return this.GetAllBankReferences(context.isClient()); }
     ...
     * @see #BankDeposit(Player, IItemHandler, BankReference, MoneyValue) for the non-shortcut version of the function.
     */
    public abstract void BankDeposit(IBankAccountMenu menu, MoneyValue requestedAmount);
    /**
     * Executes a bank deposit interaction.
     * @param player The player who is attempting to make the deposit.
     * @param container A container for items that could contain money. Used in {@link io.github.lightman314.lightmanscurrency.api.money.MoneyAPI#GetATMMoneyHandler(Player, IItemHandler)} to get the funds available for deposit.
     ...
     * @param requestedAmount The amount the player has requested to deposit. If not enough funds are available, the largest available amount will be deposited.
     */
    public abstract void BankDeposit(Player player, IItemHandler container, BankReference account, MoneyValue requestedAmount);
     ...
     */
    public final boolean BankDepositFromServer(IBankAccount account, MoneyValue amount) { return BankDepositFromServer(account, amount, true); }
    /**
     * Called by admins/commands to forcibly create and deposit money into the given bank account.
     * @param account The bank account to deposit money into.
     * @param amount The amount of money to deposit.
     ...
     */
    public abstract boolean BankDepositFromServer(IBankAccount account, MoneyValue amount, boolean notifyPlayers);
    /**
     ...
     * @see #BankWithdraw(Player, IItemHandler, BankReference, MoneyValue) for the non-shortcut version of the function.
     */
    public abstract void BankWithdraw(IBankAccountMenu menu, MoneyValue amount);
    /**
     * Executes a bank withdraw interaction.
     ...
     * @param amount The amount the player has requested to withdraw. If not enough funds are available, the largest available amount will be withdrawn.
     */
    public abstract void BankWithdraw(Player player, IItemHandler container, BankReference account, MoneyValue amount);
    ...
    public abstract Component BankTransfer(IBankAccountMenu menu, MoneyValue amount, @Nullable IBankAccount destination);
    /**
    ...
    public abstract Component BankTransfer(Player player, BankReference fromAccount, MoneyValue amount, @Nullable IBankAccount destination);
