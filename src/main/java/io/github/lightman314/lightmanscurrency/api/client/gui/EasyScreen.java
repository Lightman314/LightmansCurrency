...
import io.github.lightman314.lightmanscurrency.api.client.gui.interfaces.*;
...
public abstract class EasyScreen extends Screen implements IEasyScreen {
    ...
    private final List<ICommonTicker> guiTickers = new ArrayList<>();
    ...
    public final <T> T addChild(T child) {
        ...
        if(child instanceof ICommonTicker ticker && !this.guiTickers.contains(ticker))
    ...
    public final void removeChild(Object child) {
        ...
        if(child instanceof ICommonTicker ticker)
    ...
    public final void tick() {
        super.tick();
        for(ICommonTicker t : ImmutableList.copyOf(this.guiTickers))
            t.tick();
        this.screenTick();
    }
