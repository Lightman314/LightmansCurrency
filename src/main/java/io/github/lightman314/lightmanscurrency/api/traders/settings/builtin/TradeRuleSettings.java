...
public class TradeRuleSettings extends EasyTraderSettingsNode<TraderData> {
    ...
    public void saveSettings(SavedSettingData.MutableNodeAccess data) {
        saveRuleSettings(data,this.trader.getRules(),this.registryAccess());
    }

    @Override
    public void loadSettings(SavedSettingData.NodeAccess data, LoadContext context) {
        if(loadRuleSettings(data,this.trader.getRules(),this.registryAccess()))
            this.trader.markTradeRulesDirty();
    }

    public static void saveRuleSettings(SavedSettingData.MutableNodeAccess data, List<TradeRule> rule, HolderLookup.Provider lookup)
    {
        CompoundTag tag = new CompoundTag();
        TradeRule.saveRules(tag,rule,"Rules",lookup);
        data.setCompoundValue("trade_rules",tag);
    }

    public static boolean loadRuleSettings(SavedSettingData.NodeAccess data, List<TradeRule> rules, HolderLookup.Provider lookup)
    {
        if(data.hasCompoundValue("trade_rules"))
        {
            CompoundTag tag = data.getCompoundValue("trade_rules");
            List<TradeRule> duplicateRules = TradeRule.loadRules(tag,"Rules",null,lookup);
            for(TradeRule dupe : duplicateRules)
            {
                TradeRule actual = TradeRule.getRule(dupe.type.type,rules);
                if(actual != null)
                    actual.load(dupe.save(lookup),lookup);
            }
            return true;
        }
        return false;
