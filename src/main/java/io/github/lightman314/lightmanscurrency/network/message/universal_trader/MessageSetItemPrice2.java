...
public class MessageSetItemPrice2 {
	...
	public static MessageSetItemPrice2 decode(FriendlyByteBuf buffer) {
		return new MessageSetItemPrice2(buffer.readUUID(), buffer.readInt(), new CoinValue(buffer.readNbt()), buffer.readUtf(ItemTradeData.MAX_CUSTOMNAME_LENGTH), buffer.readUtf(ItemTradeData.MaxTradeTypeStringLength()));
	}
