	...
	private static class H extends Handler<CPacketCreatePersistentAuction>
	{
		...
		protected void handle(@Nonnull CPacketCreatePersistentAuction message, @Nullable ServerPlayer sender) {
			if(LCAdminMode.isAdminPlayer(sender))
			{
				...
				if(!generateID) {
					...
					for(int i = 0; i < persistentAuctions.size(); ++i)
					{
						...
						if(auctionData.has("id") && auctionData.get("id").getAsString().equals(message.id))
						{
							...
							EasyText.sendMessage(sender, EasyText.translatable("lightmanscurrency.message.persistent.auction.overwrite", message.id));
							return;
						}
					}
					...
					TraderSaveData.setPersistentTraderSection(TraderSaveData.PERSISTENT_AUCTION_SECTION, persistentAuctions);
					EasyText.sendMessage(sender, EasyText.translatable("lightmanscurrency.message.persistent.auction.add", message.id));
					return;
				}
				else
				{
					...
					for(int i = 1; i < Integer.MAX_VALUE; ++i)
					{
						...
						if(knownIDs.stream().noneMatch(id -> id.equals(genID)))
						{
							...
							TraderSaveData.setPersistentTraderSection(TraderSaveData.PERSISTENT_AUCTION_SECTION, persistentAuctions);
							EasyText.sendMessage(sender, EasyText.translatable("lightmanscurrency.message.persistent.auction.add", genID));
							return;
						}
					}
				...
				EasyText.sendMessage(sender, EasyText.translatable("lightmanscurrency.message.persistent.auction.fail"));
		}
	}
