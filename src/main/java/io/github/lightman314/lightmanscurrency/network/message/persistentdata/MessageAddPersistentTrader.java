...
public class MessageAddPersistentTrader {
	...
	public static void handle(MessageAddPersistentTrader message, Supplier<Context> supplier) {
		supplier.get().enqueueWork(() -> {
						...
						try {
							...
							for(int i = 0; i < persistentTraders.size(); ++i)
							{
								...
								if(traderData.has("id") && traderData.get("id").getAsString().equals(message.id))
								{
									...
									player.sendSystemMessage(Component.translatable("lightmanscurrency.message.persistent.trader.overwrite", message.id));
									return;
								}
							}
							...
							TraderSaveData.setPersistentTraderSection(TraderSaveData.PERSISTENT_TRADER_SECTION, persistentTraders);
							player.sendSystemMessage(Component.translatable("lightmanscurrency.message.persistent.trader.add", message.id));
							return;
						...
						try {
							...
							for(int i = 1; i < Integer.MAX_VALUE; ++i)
							{
								...
								if(knownIDs.stream().noneMatch(id -> id.equals(genID)))
								{
									...
									TraderSaveData.setPersistentTraderSection(TraderSaveData.PERSISTENT_TRADER_SECTION, persistentTraders);
									player.sendSystemMessage(Component.translatable("lightmanscurrency.message.persistent.trader.add", genID));
									return;
								}
							}
			...
			player.sendSystemMessage(Component.translatable("lightmanscurrency.message.persistent.trader.fail"));
		});
